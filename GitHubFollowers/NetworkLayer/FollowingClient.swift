...
extension HTTPClient: FollowingService {
      func getFollowingsList(for username: String, pageNumber: Int, followingCount: Int) async throws -> [Following] {
        let urlRequest = try makeUrlRequest(baseUrl: URL(string: baseUrl)!, path: "/users/\(username)/following", httpMethod: .get, queryParameters: [
          "per_page":"\(followingCount)",
          "page":"\(pageNumber)"
        ])
...
class FollowingCore: FollowingCoreProtocol {
    ...
    init(localDataSource: FollowingDB? = nil, remoteDataSource: FollowingService) {
